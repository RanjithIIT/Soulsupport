...
class _AddNewBusPageState extends State<AddNewBusPage> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (schoolResponse.success && schoolResponse.data != null) {
        ...
        if (data is Map) {
          ...
          if (schoolData is Map) {
            // Try to get school_id first, then fall back to id
      ...
      // Create bus
      final busResponse = await _apiService.post(Endpoints.buses, body: busData);
      ...
      setState(() {
        ...
        _showSuccess = false;
        _showError = true;
        _errorMessage = 'Error adding bus: ${e.toString()}';
      });
    }
  }
                              ...
                              const Center(
                                child: Column(
                                  children: [
                                    ...
                                    SizedBox(height: 10),
                                    Text(
                                      'Enter bus details, driver information, and route',
                                      style: TextStyle(
                                        color: Colors.grey,
                                        fontSize: 16,
                                      ),
