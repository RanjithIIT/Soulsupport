...
class _MyClassesPageState extends State<MyClassesPage> {
  ...
  void _loadClasses() async {
    try {
      final data = await ApiService.fetchTeacherClasses();
      setState(() {
        classes = data.map((c) {
          ...
          return {
            'id': c['id'],
            'name': c['name'] ?? 'No Name',
            'subject': c['department'] != null ? (c['department']['name'] ?? 'General') : 'General',
            'icon': _getIconForSubject(c['name'] ?? ''),
            'students': c['student_count'] ?? 0,
            'attendance': 0, // Backend might not provide avg attendance here
            'schedule': 'Not Set',
            'room': 'N/A',
            'section': c['section'] ?? '',
            'color': _getColorForIndex(data.indexOf(c)),
          };
        }).toList();
        isLoading = false;
      });
  ...
  String _getIconForSubject(String name) {
    ...
    if (name.contains('History')) return '🏛️';
  ...
  Color _getColorForIndex(int index) {
    if (classColors.isEmpty) return Colors.blue;
