...
class _StudentDashboardScreenState extends State<StudentDashboardScreen> {
  ...
  void _loadData() async {
    if (widget.classId == null) {
      setState(() {
        students = _generateMockStudents();
        isLoading = false;
      });
      return;
    }
    ...
    try {
      final data = await ApiService.fetchClassStudents(widget.classId!);
      setState(() {
        students = data.map((cs) {
          final s = cs['student'] ?? {};
          return {
            'id': s['student_id'] ?? s['id']?.toString() ?? 'N/A',
            'name': s['student_name'] ?? 'Unknown',
            'attendance': (s['attendance'] as num?)?.toInt() ?? 0,
            'grade': (s['grade'] as num?)?.toDouble() ?? 0.0,
            'status': 'present',
            'avatar': s['student_name'] != null ? s['student_name'][0] : 'U',
            'color': Colors.blue,
          };
        }).toList();
        isLoading = false;
  ...
  List<Map<String, dynamic>> _generateMockStudents() {
    ...
    for (int i = 0; i < widget.studentsCount; i++) {
      ...
      final String status = random.nextDouble() > 0.1 ? 'present' : 'absent';
  ...
  void _addGrade(BuildContext context, Map<String, dynamic> student) {
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    classInfo.subject, // Still using mock subject
                    style: const TextStyle(
                  ...
                  Row(
                    children: [
                      ...
                      _buildClassDetailItem(
                        'Average Attendance',
                        '${classInfo.attendance}%', // Still using mock attendance
                      ),
                    ],
                  ),
                ],
...
class StudentCard extends StatelessWidget {
  final Map<String, dynamic> student; // Changed type to Map<String, dynamic>
