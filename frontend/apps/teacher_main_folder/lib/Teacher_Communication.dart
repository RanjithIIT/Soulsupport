...
class _TeacherCommunicationScreenState extends State<TeacherCommunicationScreen> {
  ...
  static const List<String> _availableSections = ['A', 'B', 'C', 'D'];
  ...
  List<ChatContact> get _filteredContacts {
    final filtered = _contacts.values.where((contact) {
      ...
      // Section filter - normalize comparison
      if (_selectedSectionFilter != null) {
        final contactSection = (contact.section ?? '').trim().toUpperCase();
        final filterSection = _selectedSectionFilter!.trim().toUpperCase();
        if (contactSection != filterSection) return false;
      }
  ...
  List<ChatGroup> get _filteredGroups {
    return _groups.values.where((group) {
      ...
      // Section filter - normalize comparison
      if (_selectedSectionFilter != null) {
        final groupSection = (group.section ?? '').trim().toUpperCase();
        final filterSection = _selectedSectionFilter!.trim().toUpperCase();
        if (groupSection != filterSection) return false;
      }
      return true;
                            ...
                            child: Container(
                              ...
                              child: DropdownButton<String>(
                                ...
                                items: [
                                  ...
                                  ..._availableSections.map((g) => 
                                    DropdownMenuItem(value: g, child: Text(g))),
                                ],
                                onChanged: (v) => setState(() => _selectedSectionFilter = v),
                              ),
                            ),
                          ),
                        ],
                ...
                Expanded(
                  child: Container(
                    ...
                    child: DropdownButton<String>(
                      ...
                      isExpanded: true,
                      underline: const SizedBox(),
                      items: _TeacherCommunicationScreenState._availableSections.map((g) => 
                ...
                children: [
                  ...
                  Text(
                    widget.contact.type == ContactType.student
                        ? '${widget.contact.className ?? ''} ${widget.contact.section ?? ''}'.trim().isEmpty
                            ? 'Student'
                            : '${widget.contact.className ?? ''} ${widget.contact.section ?? ''}'.trim()
