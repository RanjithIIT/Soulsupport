...
class StudentSerializer(serializers.ModelSerializer):
    ...
    def get_attendance(self, obj):
        try:
            ...
            if total == 0:
                return 100 # Default high for new students to avoid demotivation or just 0
    ...
    def get_grade(self, obj):
        try:
            ...
            avg = Grade.objects.filter(student=obj).aggregate(Avg('marks_obtained'))['marks_obtained__avg']
            if avg is None:
                return 0.0
    ...
    def get_awards(self, obj):
        ...
        try:
            ...
            # Award stores comma-separated student IDs in student_ids field
            # Filter awards that contain this student's ID
            student_id = str(obj.student_id) if obj.student_id else str(obj.id)
            
            # Simple contains check might have false positives (e.g. searching for "1" finds "10", "21")
            # Ideally we would split and check exact match, but for now we use contains
            # and verify in python or assume IDs are unique enough/formatted
            
            # Using partial match for now as IDs are typically specific strings
            awards = Award.objects.filter(student_ids__contains=student_id).order_by('-date')
            ...
            return [{
                ...
                'document': award.document.url if (award.document and hasattr(award.document, 'url')) else None
            } for award in awards]
        except Exception as e:
            # Just return empty list on error to catch migration/import issues
            return []
