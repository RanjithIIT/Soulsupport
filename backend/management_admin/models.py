...
class Teacher(models.Model):
    ...
    class_teacher_section = models.CharField(max_length=10, null=True, blank=True, help_text='Section for class teacher assignment (e.g., A, B, C, D)')
...
class Student(models.Model):
    ...
    section = models.CharField(max_length=50, null=True, blank=True, help_text="Section of the student")
...
class NewAdmission(models.Model):
    ...
    section = models.CharField(max_length=1, choices=GRADE_CHOICES, null=True, blank=True, help_text="Section of the student (A, B, C, or D)")
    address = models.TextField(default="Address not provided")
    category = models.CharField(max_length=20, choices=CATEGORY_CHOICES, default='General')
    ...
    def create_student_from_admission(self):
        ...
        with transaction.atomic():
            ...
            if existing_student:
                ...
                # NEW INTEGRATION FOR TEACHER PORTAL: Automatically assign to class/section
                try:
                    from teacher.models import Class, ClassStudent
                    # Clean the class name (remove "Class " prefix if present)
                    # Robust class name matching
                    clean_class_name = self.applying_class.replace("Class ", "").replace("Grade ", "").strip()
                    
                    from .utils import get_current_academic_year
                    current_year = get_current_academic_year()
                    
                    # Look for the class in the teacher app for the current academic year
                    from django.db.models import Q
                    target_class = Class.objects.filter(
                        Q(name=self.applying_class) | Q(name=clean_class_name) | Q(name=f"Class {clean_class_name}"),
                    ...
                    if target_class:
                        ClassStudent.objects.get_or_create(
                            ...
                            defaults={'school_id': self.school_id}
                        )
                        logger.info(f"Existing student {existing_student.student_id} automatically assigned/verified in class {target_class}")
                except Exception as e:
                    logger.error(f"Error automatically assigning existing student to class: {str(e)}")
            ...
            try:
                ...
                try:
                    ...
                    if target_class:
                        ...
                        logger.info(f"Student {student.student_id} automatically assigned to class {target_class}")
                except Exception as e:
                    logger.error(f"Error automatically assigning student to class: {str(e)}")
...
class Fee(models.Model):
    ...
    section = models.CharField(max_length=50, help_text='Section of the student (A, B, C, D)')
