...
try {
    ...
    if ($portOutput) {
        ...
        if ($pidString -match '^\d+$') {
            $procId = [int]$pidString
            Write-Host "Found process $procId using port 8000, terminating..." -ForegroundColor Yellow
            try {
                ...
                Write-Host "Trying taskkill for process $procId..." -ForegroundColor Yellow
                $null = & taskkill /PID $procId /F 2>&1
                Start-Sleep -Seconds 1
            }
        }
    }
